...
def run_audit(repo_path: str, ledger_dir: str = "./grus-ledger",
              ...
              allow_ephemeral: bool = False) -> int:
    ...
    r10 = issue_certification(stage_results, audit_id, private_key_path=private_key_path, allow_ephemeral=allow_ephemeral)
    if not r10.passed:
...
def main():
    ...
    ap.add_argument("--private-key", default=None,
                    help="Path to LLC ed25519 private key (default: $GRUS_PRIVATE_KEY_PATH or /etc/grus/private_key.pem)")
    ap.add_argument("--allow-ephemeral", action="store_true",
                    help="If no private key found, generate an ephemeral keypair (dev/CI only — produces non-canonical seal)")
    args = ap.parse_args()
